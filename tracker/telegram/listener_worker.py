...
def get_active_channels():
    ...
    return list(
        SourceChannel.objects.all().filter(
...
def save_post(channel, message):
    from tracker.models import SourceChannel, Post
...
async def fetch_new_posts(client: TelegramClient):    
    ...
    for channel in channels:
        ...
        if last_time is None:
            ...
            async for message in client.iter_messages(channel, offset_date=last_time, reverse=True):
